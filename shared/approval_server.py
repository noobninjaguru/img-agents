...
def approve():
    ...
    print(f"  Score published: {score_str} ({label})", flush=True)
...
<body style="font-family:Georgia;text-align:center;padding:60px;background:#f8f8f6">
  ...
  <p>Today's score: <strong>{score_str} ({label})</strong> is live on indianmarketguru.com</p>
</body>
</html>
"""
...
@app.route("/")
def index():
    return jsonify({"service": "Indian Market Guru Agents", "status": "running"})

def start(threaded=True):
    port = int(os.environ.get("PORT", 8080))
    print(f"  ✓ Approval server running on port {port}", flush=True)
    if threaded:
        t = threading.Thread(
